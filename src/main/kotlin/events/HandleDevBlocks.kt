...
fun placeDevBlock(event: PlayerBlockPlaceEvent) {
    ...
    when (event.block) {
        ...
        Block.TARGET, Block.DIAMOND_BLOCK, Block.LAPIS_BLOCK, Block.OAK_PLANKS -> {
            width = 4
        }
                ...
                for (sy in y..y + 2) {
                    for (sz in z..128) {
                        event.player.instance.setBlock(
                            Vec(sx.toDouble(), sy.toDouble(), (sz + width).toDouble()),
                            iter.next()
                        )
                    }
                }
...
fun breakDevBlock(event: PlayerBlockBreakEvent) {
    ...
    when (event.block.getTag(Tag.String("codeBlockType"))) {
        "block" -> {
            ...
            if (event.player.instance.getBlock(Vec(x.toDouble(), y.toDouble(), (z + 1).toDouble()))
                            ...
                            for (sx in x - 1..x + 1) {
                                for (sy in y..y + 2) {
                                    for (sz in otherZ + 2..128) {
                                        blocks.add(
                                            event.player.instance.getBlock(
                                                Vec(
                                                    sx.toDouble(),
                                                    sy.toDouble(),
                                                    sz.toDouble()
                                        ...
                                        event.player.instance.setBlock(
                                            Vec(sx.toDouble(), sy.toDouble(), sz.toDouble()),
                                            Block.AIR
                                        )
                                    }
                                }
...
fun signWithLines(vararg lines: String): Block {
    val line1 = if (lines.isNotEmpty()) {
        lines[0]
    } else {
        ""
    }
    val line2 = if (lines.size >= 2) {
        lines[1]
    } else {
        ""
    }
    val line3 = if (lines.size >= 3) {
        lines[2]
    } else {
        ""
    }
    val line4 = if (lines.size >= 4) {
        lines[3]
    } else {
        ""
    }
        ...
        .withNbt(
            NBT.Compound(
                mapOf(
                    ...
                    "front_text" to NBT.Compound(
                        mapOf(
                            "color" to NBT.String("black"),
                            "has_glowing_text" to NBT.Boolean(false),
                            "messages" to NBT.List(
                                NBTType.TAG_String,
                                NBT.String("""{"text":"$line1"}"""),
                                NBT.String("""{"text":"$line2"}"""),
                                NBT.String("""{"text":"$line3"}"""),
                                NBT.String("""{"text":"$line4"}""")
                            )
                        )
                    ),
                    "back_text" to NBT.Compound(
                        mapOf(
                            "color" to NBT.String("black"),
                            "has_glowing_text" to NBT.Boolean(false),
                            "messages" to NBT.List(
